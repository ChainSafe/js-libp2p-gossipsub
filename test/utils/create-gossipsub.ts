 ...
 */
export async function startNode(gs) {
 ...
 */
export async function stopNode(gs) {
  await gs._libp2p.stop()
  await gs.stop()
}

export async function connectGossipsub(gs1, gs2) {
 ...
 */
export async function createGossipsubs({ number = 1, started = true, options = {} } = {}) {
 ...
 */
export async function tearDownGossipsubs(gss) {
  await Promise.all(
    gss.map(async (p) => {
      await p.stop()
 ...
 * @param {Gossipsub[]} gss
 */
export async function connectGossipsubs(gss) {
  for (let i = 0; i < gss.length; i++) {
 ...
 */
export async function createConnectedGossipsubs({ number = 2, options = {} } = {}) {
