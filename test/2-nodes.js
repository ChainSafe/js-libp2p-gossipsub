...
describe('basics between 2 nodes', () => {
  describe('fresh nodes', () => {
	...
	expect(gsA.subscriptions.size).to.be.eql(0)
	expect(gsB.peers.size).to.be.eql(0)
	expect(gsB.subscriptions.size).to.be.eql(0)
   ...
   it('Dial gossipNodeA into floodNodeA', (done) => {
    ...
    it('Subscribe to a topic:Z in nodeA', (done) => {
      fsA.subscribe('Z')
      fsB.once('gossipsub:subscription-change', (changedPeerInfo, changedTopics, changedSubs) => {
  ...
  describe('nodes send state on connection', () => {
    ...
    before((done) => {
      parallel([
        ...
        fsA = new GossipSub(nodeA)
        fsB = new GossipSub(nodeB)
    ...
    it('existing subscriptions are sent upon peer connection', (done) => {
      parallel([
        cb => fsA.once('gossipsub:subscription-change', () => cb()),
        cb => fsB.once('gossipsub:subscription-change', () => cb())
      ], () => {
        expect(fsA.peers.size).to.equal(1)
        expect(fsB.peers.size).to.equal(1)
